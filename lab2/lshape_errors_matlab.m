...
problem1.a = @(u, du, v, dv) dot(du, dv);
problem1.f = @(x) 0;
problem1.g = @g;

problem2.a = @(u, du, v, dv) dot(du, dv);
problem2.f = @(x) 3;
problem2.g = @(x) (x(1) == -1) * 1 + (x(2) == 1) * (-1);

problems = [problem1, problem2];
tiledlayout(2,2)
for p=1:2
    for problem_number=1:length(problemskk)
        [U_ref, Bx, By] = solve(32, p, problems(problem_number));
        ns = [1, 2, 4, 8, 16];
        h1Errors = zeros(1, length(ns));
        l2Errors = zeros(1, length(ns));
        
        for i=1:length(ns)
            [U, bx, by] = solve(ns(i), p, problems(problem_number));
            u_H = make_it_function(U, bx, by);

            l2Errors(i) = normfn_diff(u_H, U_ref, @normL2, Bx, By);
            h1Errors(i) = normfn_diff(u_H, U_ref, @normH1, Bx, By);
        end
        
        nexttile
        plot(ns, l2Errors, ns, h1Errors);
        title( sprintf("Task %d dla p=%d", problem_number, p) );
...
function [U, bx, by] = solve(n, p, problem)
    a = problem.a;
    f = problem.f;
    g = problem.g;
