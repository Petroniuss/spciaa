% Input data
knotx = simple_knot(50, 2);       % knotx vector
knoty = simple_knot(5, 2);       % knoty vector
epsilon = 0.03;                    % diffusion coefficient
...
% Setup
px = degree_from_knot(knotx);
py = degree_from_knot(knoty);
k = px + 1;

how_many_dense = 8;

pointsx = prepare_dense_linspace(0, 1, 6*epsilon, max(knotx) + 1 - how_many_dense, how_many_dense);
pointsy = linspace(0, 1, max(knoty) + 1);

bx = basis1d(px, pointsx, knotx);
by = basis1d(py, pointsy, knoty);
...
function lin=prepare_dense_linspace(start_value, end_value, epsilon, how_many_points_in_first_phase, how_many_points_in_second_phase)
    first_phase_start = start_value;
    first_phase_end = end_value-epsilon*(end_value-start_value);
    second_phase_start = first_phase_end+epsilon/how_many_points_in_second_phase;
    second_phase_end = end_value;
    first_phase_points = linspace(first_phase_start, first_phase_end, how_many_points_in_first_phase);
    second_phase_points = linspace(second_phase_start, second_phase_end, how_many_points_in_second_phase);
    lin = cat(2, first_phase_points, second_phase_points); 
end
    ...
    for q = quad_data1d(e, k, b)
      ...
      for i = dofs_on_element1d(e, b)
        ...
        for j = dofs_on_element1d(e, b)
          u = basis(j);
          M(i + 1, j + 1) = M(i + 1, j + 1) + u * v * q.w * J;
        end

        F(i + 1) = F(i + 1) + f(q.x) * v * q.w * J;
      end
    end
  end
